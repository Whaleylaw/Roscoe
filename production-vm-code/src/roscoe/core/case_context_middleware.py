        ...
        for name, project in self.name_to_project.items():
            if name in query_cleaned:
                ...
                if case_info and project not in [c.get('project_name') for c in detected_cases]:
                    # Prioritize multi-word matches (full names) over single-word matches
                    # Multi-word exact match: score=200 (e.g., "james lanier")
                    # Single-word exact match: score=100 (e.g., "lanier")
                    word_count = len(name.split())
                    match_score = 200 if word_count >= 2 else 100
                    ...
                    logger.info(f"[CASE CONTEXT] Exact match: '{name}' -> {project} (score: {match_score}, words: {word_count})")
                        ...
                        for case in self.caselist:
                            ...
                            if client_name.lower() == match_name:
                                ...
                                if project and project not in [c.get('project_name') for c in detected_cases]:
                                    ...
                                    logger.info(f"[CASE CONTEXT] Fuzzy match: '{potential}' -> {project} (score: {score}, words: {word_count})")
                                break
        ...
        for case in self.caselist:
            ...
            if project_lower in query_cleaned.replace('-', ' ').replace('_', ' '):
                if project not in [c.get('project_name') for c in detected_cases]:
                    word_count = len(project.replace('-', ' ').split())
                    detected_cases.append({
                        'project_name': project,
                        'client_name': case.get('client_name', ''),
                    ...
                    logger.info(f"[CASE CONTEXT] Project name match: {project} (words: {word_count})")

        # Sort by match specificity and limit
        # Prioritize: 1) Higher match score, 2) More words (full names), 3) Longer strings
        # This ensures "james lanier" (200 points, 2 words) beats "james" (100 points, 1 word)
        detected_cases.sort(
            key=lambda x: (
                x['match_score'],                    # Primary: match score (200 > 100)
                x.get('word_count', 1),               # Secondary: word count (2 > 1)
                len(x.get('matched_term', ''))        # Tertiary: string length
            ),
            reverse=True
        )

        # Log final selected cases for debugging
        selected = detected_cases[:self.max_cases]
        if selected:
            logger.info(f"[CASE CONTEXT] Final selected cases (top {len(selected)}): {[(c['client_name'], c['match_score'], c.get('word_count', 1)) for c in selected]}")
