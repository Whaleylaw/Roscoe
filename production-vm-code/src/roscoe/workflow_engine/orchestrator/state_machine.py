...
Vendored from Roscoe_runtime/workflow_engine/orchestrator/state_machine.py
"""
...
# Import CaseData adapter - use relative import within vendored package
try:
    from .._adapters.case_data import CaseData, get_workspace_path
except ImportError:
    CaseData = None
...
# Workflow ID aliases: old ID -> new schema ID
# Allows backward compatibility with existing case states
WORKFLOW_ID_ALIASES = {
    "open_bi_claim": "insurance_bi_claim",
    "open_pip_claim": "insurance_pip_claim",
    "open_insurance_claims": "insurance_bi_claim",
...
class StateMachine:
    ...
    def _normalize_workflow_id(self, workflow_id: str) -> str:
        """Normalize workflow ID using aliases for backward compatibility."""
    ...
    def validate_workflow_against_data(
        ...
        Args:
            ...
            case_state: Optional full case state for document checks
        ...
        # Normalize workflow name for aliased IDs
        normalized_name = self._normalize_workflow_id(workflow_name)
        ...
        if not wf_rules:
            wf_rules = workflow_derivations.get(normalized_name, {})
        ...
        # OPEN BI CLAIM workflow (handle aliases)
        elif workflow_name in ["open_bi_claim", "open_insurance_claims", "insurance_bi_claim"]:
        ...
        # OPEN PIP CLAIM workflow (handle aliases)
        elif workflow_name in ["open_pip_claim", "insurance_pip_claim"]:
        ...
        # NEGOTIATION workflow (handle aliases)
        elif workflow_name in ["bi_negotiation", "negotiate_claim"]:
            bi_claims = case_data.bi_claims
            if bi_claims:
    ...
    def _get_data_evidence(self, workflow_name: str, case_data: 'CaseData', case_state: Optional[Dict] = None) -> str:
        ...
        # Handle aliases
        if workflow_name in ["open_bi_claim", "open_insurance_claims", "insurance_bi_claim"]:
            bi_claims = case_data.bi_claims
    ...
    def load_litigation_data(self, project_name: str) -> Optional[Dict]:
        ...
        # First check project-specific file
    ...
    def compute_sol_status(
        ...
        Returns:
            Dict with status, days_remaining, sol_date, etc.
        """
        if not case_data:
            return {"status": "unknown", "message": "No case data available"}

        # Check if complaint has been filed - SOL is fulfilled
        ...
        # Continue with normal SOL calculation
    ...
    def get_case_status(self, case_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Compute SOL status with litigation check
        sol_status = None
        if case_data:
        ...
        return {
            ...
            "pending_phase_change": phase_suggestion
        }
    ...
    def _check_phase_complete(self, case_state: Dict, phase: str):
        """Check if a phase is complete (does not auto-advance)."""
        phase_def = self.phase_defs["phases"].get(phase, {})
        phase_state = case_state.get("phases", {}).get(phase, {})
        ...
        # Phase completion is now handled via suggest_phase_change() + approve_phase_change()
    ...
    def format_status_for_agent(self, status: Dict) -> str:
        ...
        # Header
        lines.append(f"**{status.get('client_name', 'Unknown')}** - Case {status.get('case_id', 'Unknown')}")
        lines.append(f"Phase: {status.get('phase_display_name', status.get('current_phase', 'Unknown'))} ({status.get('phase_progress', 0)}% complete)")
        lines.append("")
        ...
        # Completed items
        if status.get('completed_items'):
            lines.append("✅ **Completed:**")
            for item in status['completed_items'][-5:]:
                if item.get('type') == 'workflow':
                    lines.append(f"  - {item.get('name', '')} workflow")
                else:
                    lines.append(f"  - {item.get('workflow', '')}: {item.get('name', '')}")
            lines.append("")
        
        # Pending items
        if status.get('pending_items'):
            lines.append("⏳ **Waiting on:**")
            for item in status['pending_items']:
                owner = item.get('owner', 'unknown')
                blocking = " (BLOCKING)" if item.get('blocking') else ""
                lines.append(f"  - [{owner}] {item.get('description', '')}{blocking}")
            lines.append("")
        ...
        if status.get('next_actions'):
            ...
            for action in status['next_actions']:
                owner = action.owner if hasattr(action, 'owner') else action.get('owner', 'agent')
                can_automate = action.can_automate if hasattr(action, 'can_automate') else action.get('can_automate', False)
                tool_available = action.tool_available if hasattr(action, 'tool_available') else action.get('tool_available', False)
                manual_fallback = action.manual_fallback if hasattr(action, 'manual_fallback') else action.get('manual_fallback')
                description = action.description if hasattr(action, 'description') else action.get('description', '')
                prompt = action.prompt if hasattr(action, 'prompt') else action.get('prompt')
                
                auto = " (I can do this)" if can_automate and tool_available else ""
                manual = f" → Manual: {manual_fallback}" if manual_fallback and not tool_available else ""
                lines.append(f"  - [{owner}] {description}{auto}{manual}")
                if prompt:
