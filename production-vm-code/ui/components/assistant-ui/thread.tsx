...
export const Thread: FC = () => {
  ...
  const onNewChat = useCallback(() => {
    try {
      window.localStorage.removeItem("roscoe.langgraph.thread_id");
    } catch {
      // ignore
    }
    // Reset the visible thread so the user starts fresh.
    api.thread().reset();
  }, [api]);
      ...
      <MotionConfig reducedMotion="user">
        ...
        >
          <div className="flex items-center justify-between border-b px-3 py-2">
            <div className="text-sm font-medium">Roscoe</div>
