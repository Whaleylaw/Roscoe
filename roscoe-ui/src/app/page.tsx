      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="max-w-2xl text-center space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            {[
              { icon: "📋", label: "Case Cards", desc: "View rich case summaries" },
              { icon: "🏥", label: "Medical Overview", desc: "Provider files & billing" },
              { icon: "📈", label: "Timelines", desc: "Visualize medical treatment" },
              { icon: "📊", label: "Dashboards", desc: "Case overview displays" },
            ].map((item) => (
              <div
                key={item.label}
          ...
          <p className="text-zinc-500 text-sm pt-4">
            💡 Try: &quot;Show medical overview for [client name]&quot;
          </p>
        </div>
      </div>
    );
      ...
      if (status === "complete" && result?.artifact_content) {
        ...
        return (
          <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
            ...
            <p className="text-xs text-zinc-500 mt-1">
              ID: {result.artifact_id} • View in main panel →
            </p>
          </div>
        );
      }
      ...
      if (status === "complete" && result?.success && result?.data) {
        ...
        return (
          <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
            ...
            <p className="text-xs text-zinc-500 mt-1">
              {result.data.providers?.length || 0} providers • {result.data.total_billed ? `$${result.data.total_billed.toLocaleString()} billed` : ''} • View in main panel →
            </p>
          </div>
        );
      }
    ...
    render: ({ args, status, result }) => {
      ...
      if (status === "complete" && result?.success && result?.component && result?.data) {
        ...
        const getBadgeInfo = (component: string) => {
          switch (component) {
            case "CaseDashboard":
              return { icon: "📊", label: "Case Dashboard" };
            case "CaseSnapshot":
              return { icon: "📋", label: "Case Snapshot" };
            case "MedicalTreatmentOverview":
              return { icon: "🏥", label: "Medical Overview" };
            case "MedicalProviderCard":
              return { icon: "👨‍⚕️", label: "Provider Details" };
            case "InsuranceCoverageCard":
            case "InsuranceOverview":
              return { icon: "🛡️", label: "Insurance" };
            case "LienCard":
              return { icon: "📑", label: "Lien Details" };
            case "ExpenseCard":
              return { icon: "💰", label: "Expense Details" };
            case "ContactCard":
              return { icon: "👤", label: "Contact Card" };
            case "NegotiationsView":
              return { icon: "🤝", label: "Negotiations" };
            case "ComposedView":
              return { icon: "✨", label: "Custom View" };
            case "NotesView":
              return { icon: "📝", label: "Case Notes" };
            case "CalendarView":
              return { icon: "📅", label: "Calendar" };
            case "DocumentViewer":
              return { icon: "📄", label: "Document Viewer" };
            default:
              return { icon: "📄", label: component };
          }
        };
        ...
        return (
          <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
            ...
            <p className="text-xs text-zinc-500 mt-1">
              Script: {args.script_path} • View in main panel →
            </p>
          </div>
        );
      }
...
- Example: "Show medical overview for Abby Sitgraves" → show_medical_overview("Abby-Sitgraves-MVA-7-13-2024")

**For other rich UI** (case cards, timelines, dashboards):
- Use generate_ui tool with appropriate ui_type
- Examples:
  - "Show me the case card for [client]" → generate_ui with ui_type="card"
  - "Create a timeline of treatments" → generate_ui with ui_type="timeline"  
  - "Calculate the damages" → generate_ui with ui_type="dashboard"
