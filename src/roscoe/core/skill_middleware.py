...
class SkillSelectorMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        self.manifest_path = Path(manifest_path) if manifest_path else self.skills_dir / "skills_manifest.json"
    ...
    def _load_manifest(self) -> Dict:
        ...
        if not self.manifest_path.exists():
            raise FileNotFoundError(
                f"Skills manifest not found at {self.manifest_path}. "
                f"Create skills manifest file with skill definitions."
            )
    ...
    def _select_and_inject_skills(self, request):
        ...
        logger.error(f"[SKILL SELECTOR] Processing {len(messages)} messages")
        ...
        for i, msg in enumerate(reversed(messages)):
            logger.error(f"[SKILL SELECTOR] Message {i}: type={type(msg).__name__}")

            # Check if this is a HumanMessage (user message)
            if isinstance(msg, HumanMessage):
                logger.error(f"[SKILL SELECTOR] Found HumanMessage at index {i}")
                content = msg.content
                logger.error(f"[SKILL SELECTOR] Content type: {type(content)}")

                # Handle string or list content
                if isinstance(content, str):
                    user_query = content
                    logger.error(f"[SKILL SELECTOR] Content is string: '{user_query}'")
                elif isinstance(content, list):
                    logger.error(f"[SKILL SELECTOR] Content is list with {len(content)} blocks")
                    ...
                    logger.error(f"[SKILL SELECTOR] Extracted from blocks: '{user_query}'")
                else:
                    logger.error(f"[SKILL SELECTOR] Unknown content type: {type(content)}, value: {content}")
                break

        logger.error(f"[SKILL SELECTOR] Extracted user query: '{user_query}'")

        if not user_query:
            logger.error("[SKILL SELECTOR] No user query found - skipping skill selection")
            return request
        ...
        logger.error(f"[SKILL SELECTOR] Similarity scores:")
        for idx, skill in enumerate(self.manifest['skills']):
            score = scores[idx].item()
            logger.error(f"  - {skill['name']}: {score:.3f} (threshold: {self.threshold})")
        ...
        for idx in top_indices:
            score = scores[idx].item()
            logger.error(f"[SKILL SELECTOR] Checking top skill '{self.manifest['skills'][idx]['name']}' with score {score:.3f}")
            if score > self.threshold:
                skill = self.manifest['skills'][idx]
                skill_content = self._load_skill_file(skill['file'])
                logger.error(f"[SKILL SELECTOR] ✅ Selected skill: {skill['name']}")
                ...
                logger.error(f"[SKILL SELECTOR] ❌ Skill below threshold: {self.manifest['skills'][idx]['name']} ({score:.3f} <= {self.threshold})")
    ...
    def wrap_model_call(self, request, handler):
        ...
        logger.error("="*80)
        logger.error("⚡ SKILL SELECTOR MIDDLEWARE EXECUTING (SYNC) ⚡")
        logger.error("="*80)
        ...
        logger.error(f"Selected skills: {[s['name'] for s in selected_skills]}")
        return handler(modified_request)

    async def awrap_model_call(self, request, handler):
        """Asynchronous model call wrapper - injects skills before calling model."""
        logger.error("="*80)
        logger.error("⚡ SKILL SELECTOR MIDDLEWARE EXECUTING (ASYNC) ⚡")
        logger.error("="*80)
