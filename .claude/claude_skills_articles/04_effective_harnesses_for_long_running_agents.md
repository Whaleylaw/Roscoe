...
1. **Two-agent architecture**: Use an initializer agent for first-run setup and a coding agent for incremental progress
2. **Feature list file**: Comprehensive JSON file with all features marked as passing/failing prevents premature victory declarations
3. **Progress tracking**: `claude-progress.txt` file + git commits create breadcrumbs for future sessions
4. **Incremental work**: Prompt agents to work on ONE feature at a time
5. **Clean state discipline**: End each session with committed, documented, working code
6. **End-to-end testing**: Use browser automation (Puppeteer MCP) for real user-like testing
7. **Startup routine**: Always run pwd, read progress files, check git logs, and verify basic functionality before new work
