...
**Project-Specific JSON Files (`projects/{case-name}/`):**
Each project folder contains its OWN versions of these JSON files:
...
2. Read that project's `overview.json` from the project folder
...
# Backwards-compatible export (calls the function to get dynamic prompt)
# Note: For dynamic datetime, import and call get_minimal_prompt() instead
minimal_personal_assistant_prompt = get_minimal_prompt()
