...
The workflow engine uses **JSON schemas** as the canonical source for execution, and **markdown SOPs** for procedural guidance.

### Canonical execution (JSON)

- `workflow_engine/schemas/phase_definitions.json`
- `workflow_engine/schemas/workflow_definitions.json`

The state machine reads these JSON files to decide:\n- current phase\n- which workflows are active\n- which step is next\n- what is blocking\n+
### Procedural guidance (SOP markdown)

Each workflow definition includes a `sop_path` pointing to the canonical SOP file:\n- `Roscoe_workflows/workflows/**/workflows/<workflow_id>/workflow.md`\n+\n+Example: `insurance_bi_claim` → `Roscoe_workflows/workflows/phase_1_file_setup/workflows/insurance_bi_claim/workflow.md`\n+\n+When executing a workflow, the agent should:\n+1. Use the state machine for tracking/status/next actions\n+2. Open the `sop_path` for step-by-step instructions\n+
### Central discovery (skills/tools/templates)\n+\n+To keep reusable resources **outside** workflow folders while preserving workflow-local copies, we maintain central registries under:\n+\n+- `Roscoe_workflows/workflows/tools/tools_manifest.json`\n+- `Roscoe_workflows/workflows/skills/skills_manifest.json`\n+- `Roscoe_workflows/workflows/templates/templates_manifest.json`\n+\n+Workflows can reference these manifests by ID/path, while the SOP remains the human-readable procedure.
