#!/usr/bin/env python3
"""
Ingest new medical providers to FalkorDB (deduplicate existing 773).

Run inside roscoe-agents container:
  docker exec roscoe-agents python3 /deps/roscoe/src/roscoe/scripts/ingest_medical_providers_direct.py

Process:
1. Load 2,160 providers from medical_providers.json
2. Query graph for existing 773 providers
3. Create only the NEW providers (exact name match deduplication)
4. Create PART_OF relationships to HealthSystems where applicable

Expected: ~1,387 new providers
"""

import json
import os
from pathlib import Path
from falkordb import FalkorDB


def ingest_medical_providers():
    """Ingest new medical providers with deduplication."""

    # Connect to FalkorDB
    host = os.getenv("FALKORDB_HOST", "roscoe-graphdb")
    port = int(os.getenv("FALKORDB_PORT", "6379"))

    print(f"Connecting to FalkorDB at {host}:{port}")
    db = FalkorDB(host=host, port=port)
    graph = db.select_graph("roscoe_graph")
    print("✓ Connected\n")

    # Try both possible paths (gcsfuse mount and Docker mount)
    possible_paths = [
        Path("/app/workspace_paralegal/json-files/memory-cards/entities/medical_providers.json"),
        Path("/mnt/workspace/json-files/memory-cards/entities/medical_providers.json"),
    ]

    file_path = None
    for p in possible_paths:
        if p.exists():
            file_path = p
            break

    if not file_path:
        print("❌ medical_providers.json not found")
        return

    print("="*70)
    print("MEDICAL PROVIDERS INGESTION - PHASE 1e")
    print("="*70)
    print()

    # Pre-check
    result = graph.query("MATCH (n) RETURN count(n) as total")
    nodes_before = result.result_set[0][0]
    result = graph.query("MATCH ()-[r]->() RETURN count(r) as total")
    rels_before = result.result_set[0][0]

    print(f"Nodes before: {nodes_before:,}")
    print(f"Relationships before: {rels_before:,}")

    # Get existing provider names from graph
    print("\nQuerying existing providers...")
    result = graph.query("MATCH (p:MedicalProvider) RETURN p.name")
    existing_names = set(row[0] for row in result.result_set) if result.result_set else set()
    print(f"✓ Found {len(existing_names)} existing MedicalProvider nodes\n")

    # Load all providers from JSON
    with open(file_path, 'r', encoding='utf-8') as f:
        all_providers = json.load(f)

    print(f"Loaded {len(all_providers)} providers from JSON")

    # Filter to only new ones
    new_providers = [p for p in all_providers if p['name'] not in existing_names]
    print(f"After deduplication: {len(new_providers)} NEW providers to ingest\n")

    if len(new_providers) == 0:
        print("✓ All providers already exist - nothing to ingest")
        return

    stats = {
        'created': 0,
        'relationships': 0,
        'errors': [],
        'no_parent': 0
    }

    # Process new providers
    batch_size = 100
    total_batches = (len(new_providers) + batch_size - 1) // batch_size

    for i in range(0, len(new_providers), batch_size):
        batch = new_providers[i:i+batch_size]
        batch_num = (i // batch_size) + 1

        print(f"Batch {batch_num}/{total_batches} ({len(batch)} providers)...")

        for provider in batch:
            name = provider['name']
            attrs = provider.get('attributes', {})

            try:
                # Create provider node
                query = """
                MERGE (p:MedicalProvider {name: $name, group_id: $group_id})
                ON CREATE SET p += $props, p.created_at = timestamp()
                ON MATCH SET p.updated_at = timestamp()
                RETURN p.name
                """

                params = {
                    'name': name,
                    'group_id': 'roscoe_graph',
                    'props': attrs
                }

                result = graph.query(query, params)

                if result.nodes_created > 0:
                    stats['created'] += 1

                    # Create PART_OF relationship to HealthSystem if parent specified
                    parent_system = attrs.get('parent_system', '')

                    if parent_system:
                        rel_query = """
                        MATCH (p:MedicalProvider {name: $provider_name})
                        MATCH (h:HealthSystem {name: $system_name})
                        MERGE (p)-[:PART_OF]->(h)
                        """

                        rel_params = {
                            'provider_name': name,
                            'system_name': parent_system
                        }

                        try:
                            rel_result = graph.query(rel_query, rel_params)
                            if rel_result.relationships_created > 0:
                                stats['relationships'] += 1
                        except:
                            pass  # Health system may not exist yet

                    else:
                        stats['no_parent'] += 1

            except Exception as e:
                error_msg = f"{name}: {str(e)}"
                stats['errors'].append(error_msg)

        # Progress update every batch
        print(f"  → Created: {stats['created']}, Relationships: {stats['relationships']}")

    print()

    # Post-check
    result = graph.query("MATCH (n) RETURN count(n) as total")
    nodes_after = result.result_set[0][0]
    result = graph.query("MATCH ()-[r]->() RETURN count(r) as total")
    rels_after = result.result_set[0][0]

    nodes_added = nodes_after - nodes_before
    rels_added = rels_after - rels_before

    # Check Abby Sitgraves (canary)
    result = graph.query('MATCH (c:Case {name: "Abby-Sitgraves-MVA-7-13-2024"})-[r]-() RETURN count(r)')
    abby_rels = result.result_set[0][0]

    print("="*70)
    print("INGESTION COMPLETE")
    print("="*70)
    print(f"Nodes before: {nodes_before:,}")
    print(f"Nodes after: {nodes_after:,}")
    print(f"Nodes added: {nodes_added:,} (expected: ~{len(new_providers)})")
    print()
    print(f"Relationships before: {rels_before:,}")
    print(f"Relationships after: {rels_after:,}")
    print(f"Relationships added: {rels_added:,}")
    print()
    print(f"Providers created: {stats['created']}")
    print(f"PART_OF relationships: {stats['relationships']}")
    print(f"Providers without parent system: {stats['no_parent']}")
    print(f"Errors: {len(stats['errors'])}")
    print()
    print(f"Abby Sitgraves relationships: {abby_rels} (should be 93)")

    if abby_rels != 93:
        print("\n⚠️  WARNING: Abby Sitgraves case relationships changed!")
        return False

    # Verify total provider count
    result = graph.query("MATCH (p:MedicalProvider) RETURN count(p)")
    total_providers = result.result_set[0][0]
    print(f"\nTotal MedicalProvider nodes in graph: {total_providers}")
    print(f"Expected: ~{773 + len(new_providers)}")

    print("\n✅ Phase 1e complete")
    return True


if __name__ == "__main__":
    ingest_medical_providers()
