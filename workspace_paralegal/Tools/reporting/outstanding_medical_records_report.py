#!/usr/bin/env python3
"""
Outstanding Medical Records Report - Pending Records Requests

Queries medical_providers.json for records that have been requested but not yet received.
Identifies stale requests that may need follow-up.

Usage:
    python /workspace/Tools/reporting/outstanding_medical_records_report.py [--format markdown|json] [--output path]
...
def generate_report(database_path: str, format: str = "markdown"):
    ...
    try:
        ...
        # Filter for outstanding records (two categories):
        ...
        for provider in providers_data:
            ...
            if requested and requested not in ["null", "None", ""]:
                if not received or received in ["null", "None", ""]:
                    ...
                    provider_copy["record_status"] = "Requested"
            ...
            # Category 2: Done treating but no request sent
                ...
                if not requested or requested in ["null", "None", ""]:
                    provider_copy = provider.copy()
                    provider_copy["record_status"] = "Needs Request"
        ...
        for provider in outstanding:
            if provider.get("record_status") == "Requested":
                requested_date = provider.get("date_medical_records_requested")
        ...
        outstanding.sort(key=lambda x: (
            0 if x.get("record_status") == "Needs Request" else 1,
        ...
        if format == "markdown":
            ...
            return json.dumps({
                "report_type": "outstanding_medical_records",
...
def format_markdown(data):
    ...
    output = []
    output.append("# Outstanding Medical Records Report")
    output.append(f"\n**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    output.append(f"**Total Outstanding:** {len(data)}\n")

    if not data:
        output.append("*No outstanding medical records requests found.*")
        return "\n".join(output)

    # Summary statistics
    needs_request = len([p for p in data if p.get("record_status") == "Needs Request"])
    requested_data = [p for p in data if p.get("record_status") == "Requested"]
    ...
    for provider in data:
        ...
        status = provider.get("record_status", "N/A")
        requested = provider.get("date_medical_records_requested", "N/A")
        ...
        follow_up = provider.get("medical_records_follow_up_date", "N/A")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Outstanding Medical Records Report",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s
  %(prog)s --format json
  %(prog)s --output /Reports/operational/outstanding_records.md
        """
    )
    parser.add_argument(
        "--database",
